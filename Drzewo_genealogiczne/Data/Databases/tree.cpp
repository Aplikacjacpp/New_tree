...
C_tree::C_tree(N_vektor<C_human> V_human, N_vektor<C_goverment_relation> v_gover, C_human human) : C_element( human, v_gover)
{
	int i, j, t;
	
	for (i = 0; i < V_human.m_size(); i++)
	{
		for (j = 0; j < V_children.m_size(); j++)
		{
			if (V_human[i].m_set_id() == V_children[j].m_set_id())
			{
				V_human_children.m_push_back(V_human[i]);
			}
		}
		for (j = 0; j < V_grandchildren.m_size(); j++)
		{
			if (V_human[i].m_set_id() == V_grandchildren[j].m_set_id())
			{
				V_human_grandchildren.m_push_back(V_human[i]);
			}
		}
		for (j = 0; j < V_grandparents.m_size(); j++)
		{
			if (V_human[i].m_set_id() == V_grandparents[j].m_set_id())
			{
				V_human_grandparent.m_push_back(V_human[i]);
			}
		}
		for (j = 0; j < V_parent.m_size(); j++)
		{
			if (V_human[i].m_set_id() == V_parent[j].m_set_id())
			{
				V_human_parent.m_push_back(V_human[i]);
			}
		}
		for (j = 0; j < V_order.m_size(); j++)
		{
			if (V_human[i].m_set_id() == V_order[j].m_set_id())
			{
				V_human_order.m_push_back(V_human[i]);
			}
		}
		for (j = 0; j < V_partner.m_size(); j++)
		{
			if (V_human[i].m_set_id() == V_partner[j].m_set_id())
			{
				V_human_partner.m_push_back(V_human[i]);
			}
		}
		for (j = 0; j < V_sibling.m_size(); j++)
		{
			if (V_human[i].m_set_id() == V_sibling[j].m_set_id())
			{
				V_human_sibling.m_push_back(V_human[i]);			
			}
		}
	}
}
