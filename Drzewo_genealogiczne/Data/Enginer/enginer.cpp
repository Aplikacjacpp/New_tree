...
C_tree C_enginer::m_create_tree(C_id id_pointer) {
	...
	//tu przerobic na odpowiedni konstruktor
	int i;
	N_vektor<C_human> V_human;
	C_human Human(m_create_human(id_pointer));
	for (i = 1; i < i_index; i++)
	{
		V_human.m_push_back(m_create_human(i));
	}
	//	std::cin >> i;
	C_tree Tree(V_human,V_goverment_relation, Human);
...
void C_enginer::m_delete_human(C_id id_pointer) {
	int i,j,k;
	C_id Id;
	if (V_goverment_personaly.m_size() >= V_goverment_date.m_size() && V_goverment_personaly.m_size() >= V_goverment_relation.m_size())
	{
		for (i = 0; i < V_goverment_personaly.m_size(); i++)
		{
			if (id_pointer == V_goverment_personaly[i].m_set_value_id())
			{
				V_goverment_personaly.m_erase(i);
			}
			if (id_pointer == V_goverment_date[i].m_set_value_id())
			{
				V_goverment_date.m_erase(i);
			}
			if (id_pointer == V_goverment_relation[i].m_set_value_id())
			{
				V_goverment_relation.m_erase(i);
			}
		}
	}
	else if (V_goverment_date.m_size() >= V_goverment_personaly.m_size() && V_goverment_date.m_size() >= V_goverment_relation.m_size())
	{
	...
	else if (V_goverment_relation.m_size() >= V_goverment_date.m_size() && V_goverment_relation.m_size() >= V_goverment_personaly.m_size())
	{
	...
	//usuwa id z plikow!!
	for (i = 0; i < V_goverment_relation.m_size(); i++) //ponizsze dziwactwo ma usuwac id w relacjach:)
	{
		N_striing data = V_goverment_relation[i].m_set_contens(),temp;
		for (j = 0; j < data.m_size()-1; j++)
		{
			k = (int)data[j] + (int)data[j + 1];
			switch (k)
			{
			case t_children:
			...
			case t_order:
				temp.m_clear();
				for (k = j+1; k < data.m_size(); k++)
				{
					if (data[k] == 'r'||data[k]=='>')
						break;
					temp.m_push_back(data[k]);
				}
				if (temp.m_size() > 0)
				{
					Id.m_get_contens(temp);
					if (id_pointer == Id)
					{
						for(k=0; k<4;k++)
						data.m_erase(j);
					}
					V_goverment_relation[i].m_get_contens(data);
				}
			}
		}
	}
