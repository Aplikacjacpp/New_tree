...
int C_aplication_txt::m_menu_tree() {// menu drzewa
	...
	while (true)
	{
		...
		cls();
		m_create_logo();
		std::cout << "\t\t\t      Tree successfully loadeddupa!\n\n";
		SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 15);
		for (int i = 0; i < 3; ++i)
		{
			if (i == ptr)       // podswietla dana opcje na niebiesko, dopisuje strzalke
		...
		while (true)
		{
			...
			else if (GetAsyncKeyState(VK_RETURN) != 0)
			{
				...
				{
					Sleep(1500);
					return M_menu_glowne;		//eksportuj
				}
				}
				break;
			}
...
int C_aplication_txt::m_menu_relation(int where)
{
	...
	bool b_pointer, active = false, b_partner = false;
		...
		while (true)
		{
			...
			else if (GetAsyncKeyState(VK_RETURN) != 0)
			{
				...
				case 8:
				{
					if (b_partner) {
						int i;
						m_update_human(Element.m_set_Human());
						C_human Human(m_create_human(Element.m_set_partner().m_set_id()));
						std::cout << Human << "\n"; //bledy!!
						Human.m_update_date(2, Element.m_set_Human().m_set_Vdate()[2]);
						m_update_human(Human);
					}
						...
						else
						{
							...
							else
							{
								if (yy.m_size() < 4) {
									...
									if (i-13 > j) {//dodac dwa warunki ze mniejsze od daty smierci jednego z humanow oraz nie wieksze niz 100 lat od najstarszego
...
int C_aplication_txt::m_look_at(C_id id) {
	...
	C_human Human(m_create_human(id));
	for (i = 1; i < i_index; i++)
	{
	...
	//ladowanie wektorow do stworzenia interfejsu drzewa
		...
		for (i = 0; i < Tree.m_set_v_grandparents().m_size(); i++)
		{
			...
			Vektor.m_push_back(Tree.m_get_human(t_grandparent,i).m_short_interface_personaly());
		...
		{
			Vektor.m_push_back(Tree.m_get_human(t_parent, i).m_short_interface_personaly());
	...
	//std::cout<<Tree.m_set_Human(); //wyswietlenie humana wskaznikowego
		...
		{
			std::cout << "partner\n";
			Vektor.m_push_back(Tree.m_get_human(t_partner, i).m_short_interface_personaly());
		...
		{
			Vektor.m_push_back(Tree.m_get_human(t_sibling, i).m_short_interface_personaly());
		...
		{
			Vektor.m_push_back(Tree.m_get_human(t_children, i).m_short_interface_personaly());
		...
		{
			Vektor.m_push_back(Tree.m_get_human(t_grandchildren, i).m_short_interface_personaly());
